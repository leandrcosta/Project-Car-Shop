...
- Conhecimento de`Programação Orientada a Objetos`: `Herança`, `Abstração`, `Encapsulamento` e `Polimorfismo`;
- A criação e utilização de `Interfaces`;
- Implementar, em `TypeScript`: `Classes`, `Atributos`, `Métodos` e `Objetos`;
...
- `git clone git@github.com:leandrcosta/Project-Car-Shop.git`.
